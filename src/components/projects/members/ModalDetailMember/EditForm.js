        ...
        return (
            <Grid container spacing={2} style={{ paddingLeft: 3, paddingRight: 3 }} >
                <Grid item lg={12} md={12} sm={12} xs={12} align="center">
                    <Avatar alt={"Photo profile " + data.user.name} className={classes.photoProfileBg}>{data.user.name.charAt(0).toUpperCase()}</Avatar>
                    <Typography variant="h6">{data.user.name}</Typography>
                    <Typography variant="body2">{data.user.email}</Typography>
                </Grid>
                <Grid item lg={12} md={12} sm={12} xs={12} align="center">
                    {data.user.occupation ? <Typography variant="body2">{data.user.occupation.name}</Typography> : <></>}
                        <Typography variant="body2">Last login : {data.user.last_login ? moment(data.user.last_login).format('DD MMM YYYY') : ''}</Typography>
        ...
        return (
            <Grid container spacing={2} style={{ paddingLeft: 4, paddingRight: 4 }} >
                ...
                <Grid item lg={12} md={12} sm={12} xs={12} align="center">
                    {data.user.occupation ? <Typography variant="body2">{data.user.occupation.name}</Typography> : <></>}
                    <Typography variant="body2">Last login : {data.user.last_login ? moment(data.user.last_login).format('DD MMM YYYY') : ''}</Typography>
                    <Typography variant="body2">Role : {data.role.name}</Typography>
               </Grid>
            </Grid>
        )
    }
